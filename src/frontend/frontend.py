...
def extraction_tool():
    ...
    async def handle_upload(e):
        if not e.file.name.endswith('.pdf'):
        ...
        f = {
            'e': (e.file.name, file_bytes, e.file.content_type or 'application/pdf')  # 'e' must match parameter name in your FastAPI route
        }
        async with httpx.AsyncClient() as client:
            resp = await client.post('http://localhost:8000/api/pdf/ocr', files = f, timeout=None)
            if resp.status_code == 422:
                ui.notify(f"File {e.file.name} uploaded in improper format")
            else:
                ui.notify(f"file {e.file.name} uploaded")

        ocr_body = resp.json()
        if resp.is_error:
            error_message = ocr_body.get("error", {}).get("message", "Failed to process PDF")
            ui.notify(f"OCR failed: {error_message}", type='negative')
            return

        pages = ocr_body.get("pages", [])
        for page in pages:
            all_pages.append(page)
        file_name.append(e.file.name)
        # change if scaling to handle multiple PDF's
        markdown_display.set_content(ocr_body.get("markdown", ""))
         
    async def metadata_extraction() -> int:
        if not all_pages or not file_name:
            ui.notify("Please upload a PDF before extracting metadata", type='warning')
            return
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                ...
                json={'ocr_output': ocr, 'doc_name': file_name[0]},
                timeout=None,
            )
            body = resp.json()
            if resp.is_error:
                error_message = body.get("error", {}).get("message", "Metadata extraction failed")
                ui.notify(f"metadata extraction unable to be completed: {error_message}", type='negative')
                return

            metadata_dict = {
                "title": body.get("title"),
                "people_and_organizations": body.get("people_and_organizations"),
                "description": body.get("description"),
                "date": body.get("date"),
                "subject": body.get("subject"),
                "doc_name": file_name[0],
            }
            ui.notify("metadata extraction finished")
            # display to user
            json_output.properties["content"]["json"] = metadata_dict
            json_output.update()
            # add to database
            db_resp = await client.post(
                'http://localhost:8000/api/db/process',
                json=metadata_dict,
                timeout=None,
            )
            db_body = db_resp.json()
            if db_resp.is_error:
                error_message = db_body.get("error", {}).get("message", "Failed to save metadata")
                ui.notify(f"metadata could not be saved: {error_message}", type='negative')
            else:
                ui.notify("added to database")
    ...
    with ui.column().classes('w-full max-w-screen-lg mx-auto p-6 gap-y-6'):
        ...
        with ui.card().classes('w-full p-5 border border-gray-200 rounded-lg shadow-sm'):
            ui.label('Step 2: Extract Structured Metadata').classes('text-lg font-semibold')
